 ...
 */
namespace FapiMember\Library\Humbug\PhpScoper\Console;

use FapiMember\Library\Fidry\Console\Application\Application;
use FapiMember\Library\Fidry\Console\IO;
use FapiMember\Library\Humbug\PhpScoper\Autoload\ComposerFileHasher;
use FapiMember\Library\Humbug\PhpScoper\Autoload\ScoperAutoloadGenerator;
use FapiMember\Library\Humbug\PhpScoper\Configuration\Configuration;
use FapiMember\Library\Humbug\PhpScoper\Scoper\Scoper;
...
use FapiMember\Library\Humbug\PhpScoper\Throwable\Exception\ParsingException;
use FapiMember\Library\Symfony\Component\Filesystem\Filesystem;
use FapiMember\Library\Symfony\Component\Filesystem\Path;
use Throwable;
...
use function FapiMember\Library\Safe\fileperms;
...
final readonly class ConsoleScoper
{
    ...
    public function scope(IO $io, Configuration $config, array $paths, string $outputDir, bool $stopOnFailure): void
    {
        ...
        try {
            $this->doScope($config, $outputDir, $stopOnFailure, $logger);
    ...
    private function doScope(Configuration $config, string $outputDir, bool $stopOnFailure, ScoperLogger $logger): void
    {
        ...
        $this->scopeFiles($config, $files, $stopOnFailure, $logger, $symbolsRegistry);
        foreach ($excludedFilesWithContents as $excludedFileWithContent) {
            $this->dumpFileWithPermissions($excludedFileWithContent);
        }
        self::dumpScoperAutoloader($files, $excludedFilesWithContents, $symbolsRegistry);
    }
    /**
     * @param File[] $files
     ...
     * @param File[] $files
     */
    private function scopeFiles(Configuration $config, array $files, bool $stopOnFailure, ScoperLogger $logger, SymbolsRegistry $symbolsRegistry): void
    {
