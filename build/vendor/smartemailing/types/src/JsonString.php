...
use FapiMember\Library\Nette\Utils\JsonException;
...
use FapiMember\Library\SmartEmailing\Types\ExtractableTraits\ExtractableTrait;
final class JsonString implements ToStringInterface, ComparableInterface
{
     ...
     */
    public static function from($data): JsonString
    {
        if ($data instanceof self) {
            return $data;
        }
        $string = StringType::fromOrNull($data, \true);
        if (\is_string($string)) {
            return new static($string);
        }
        $array = Arrays::fromOrNull($data, \true);
        if (\is_array($array)) {
            return self::encode($data);
        }
        throw InvalidTypeException::typesError(['string', 'array'], $data);
    }
    public static function encode(mixed $value, bool $oneLine = \false): self
    {
        try {
            return new static(Json::encode($value, $oneLine ? 0 : Json::PRETTY));
