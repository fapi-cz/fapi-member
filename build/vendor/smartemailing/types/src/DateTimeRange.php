...
final class DateTimeRange implements ToArrayInterface, ComparableInterface
{
    ...
    private function __construct(array $data)
    {
        ...
        $compared = \strcmp(DateTimeFormatter::format($this->from), DateTimeFormatter::format($this->to));
        if ($compared > 0) {
            throw new InvalidTypeException(self::class . ' cannot have negative duration');
        }
        $interval = $this->to->diff($this->from);
        $this->durationInSeconds = $interval->days * 86400 + $interval->h * 3600 + $interval->i * 60 + $interval->s;
    }
    ...
    public function contains(\DateTimeInterface $dateTime): bool
    {
        ...
        return $timestamp >= $this->getFrom()->getTimestamp() && $timestamp <= $this->getTo()->getTimestamp();
    }
    /**
     * @return array<mixed>
     */
    ...
    {
        return ['from' => DateTimeFormatter::format($this->from), 'to' => DateTimeFormatter::format($this->to)];
    }
}
