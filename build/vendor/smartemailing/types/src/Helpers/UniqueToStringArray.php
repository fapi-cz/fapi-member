...
use FapiMember\Library\SmartEmailing\Types\UniqueArrayFeatures;
/*
...
final class UniqueToStringArray implements \Countable, \IteratorAggregate
{
    ...
    private function __construct(array $data = [])
    {
        ...
        foreach ($data as $value) {
            if (!$value instanceof ToStringInterface) {
                throw InvalidTypeException::typeError('all members of array must implement ' . ToStringInterface::class, $value);
            }
            $this->add($value);
        }
    }
     ...
     */
    public static function intersect(array $uniqueToStringArrays): self
    {
     ...
     * @return \SmartEmailing\Types\Helpers\UniqueToStringArray
     */
    public static function union(array $uniqueIntArrays): self
    {
        $result = [];
    ...
    public function add(ToStringInterface $valueObject): bool
    {
        ...
        if ($this->type !== $type) {
            throw InvalidTypeException::typeError('all members of array must be of type ' . $this->type, $valueObject);
        }
    ...
    public function remove(ToStringInterface $valueObject): void
    {
        $key = $valueObject->__toString();
    ...
    public function contains(ToStringInterface $valueObject): bool
    {
        $key = $valueObject->__toString();
