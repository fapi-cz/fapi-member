...
class Session
{
    ...
    private const SecurityOptions = [
        ...
        'use_cookies' => 1,
        // must be enabled to prevent Session Hijacking and Fixation
        'use_only_cookies' => 1,
        // must be enabled to prevent Session Fixation
        'use_trans_sid' => 0,
        // must be disabled to prevent Session Hijacking and Fixation
        'use_strict_mode' => 1,
        // must be enabled to prevent Session Fixation
        'cookie_httponly' => \true,
    ];
    ...
    private $options = [
        ...
        'cookie_lifetime' => 0,
        // for a maximum of 3 hours or until the browser is closed
    ...
    private function doStart($mustExists = \false): void
    {
        ...
        if (!session_id()) {
            // session is started for first time
            $id = $this->request->getCookie(session_name());
            $id = (is_string($id) && preg_match('#^[0-9a-zA-Z,-]{22,256}$#Di', $id)) ? $id : session_create_id();
            session_id($id);
            // causes resend of a cookie to make sure it has the right parameters
        }
        try {
    ...
    {
        return session_status() === \PHP_SESSION_ACTIVE || $this->fileExists && $this->request->getCookie($this->getName());
    }
    /**
    ...
    {
        $normalized = [];
        $allowed = ini_get_all('session', \false) + ['session.read_and_close' => 1, 'session.cookie_samesite' => 1];
        // for PHP < 7.3
        foreach ($options as $key => $value) {
            ...
            $normKey = strtolower(preg_replace('#(.)(?=[A-Z])#', '$1_', $key));
        ...
        $this->autoStart = $normalized['auto_start'] ?? \true;
    ...
    private function configure(array $config): void
    {
        ...
        if ($cookie !== $origCookie) {
            if (\PHP_VERSION_ID >= 70300) {
                ...
                // @ may trigger warning when session is active since PHP 7.2
            } else {
    ...
    {
        if ($expire === null) {
            return $this->setOptions(['gc_maxlifetime' => self::DefaultFileLifetime, 'cookie_lifetime' => 0]);
        } else {
            $expire = Nette\Utils\DateTime::from($expire)->format('U') - time();
            return $this->setOptions(['gc_maxlifetime' => $expire, 'cookie_lifetime' => $expire]);
        }
    }
    /**
    ...
    {
        return $this->setOptions(['cookie_path' => $path, 'cookie_domain' => $domain, 'cookie_secure' => $secure, 'cookie_samesite' => $sameSite]);
    }
    /** @deprecated */
    ...
    {
        return $this->setOptions(['save_path' => $path]);
    }
    /**
    ...
    private function sendCookie(): void
    {
        ...
        $this->response->setCookie(session_name(), session_id(), $cookie['lifetime'] ? $cookie['lifetime'] + time() : 0, $cookie['path'], $cookie['domain'], $cookie['secure'], $cookie['httponly'], $cookie['samesite'] ?? null);
    }
}
