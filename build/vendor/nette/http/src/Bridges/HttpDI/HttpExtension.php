...
namespace FapiMember\Library\Nette\Bridges\HttpDI;

use FapiMember\Library\Nette;
use FapiMember\Library\Nette\Schema\Expect;
/**
...
class HttpExtension extends Nette\DI\CompilerExtension
{
    ...
    public function getConfigSchema(): Nette\Schema\Schema
    {
        return Expect::structure([
            ...
            'headers' => Expect::arrayOf('scalar|null')->default(['X-Powered-By' => 'Nette Framework 3', 'Content-Type' => 'text/html; charset=utf-8'])->mergeDefaults(),
            'frames' => Expect::anyOf(Expect::string(), Expect::bool(), null)->default('SAMEORIGIN'),
            // X-Frame-Options
            'csp' => Expect::arrayOf('array|scalar|null'),
            // Content-Security-Policy
            'cspReportOnly' => Expect::arrayOf('array|scalar|null'),
            // Content-Security-Policy-Report-Only
            'featurePolicy' => Expect::arrayOf('array|scalar|null'),
            // Feature-Policy
            'cookiePath' => Expect::string()->dynamic(),
            'cookieDomain' => Expect::string()->dynamic(),
            'cookieSecure' => Expect::anyOf('auto', null, \true, \false)->firstIsDefault()->dynamic(),
            // Whether the cookie is available only through HTTPS
            'disableNetteCookie' => Expect::bool(\false),
        ]);
    }
    public function loadConfiguration()
    {
        ...
        $config = $this->config;
        $builder->addDefinition($this->prefix('requestFactory'))->setFactory(Nette\Http\RequestFactory::class)->addSetup('setProxy', [$config->proxy]);
        $request = $builder->addDefinition($this->prefix('request'))->setFactory('@Nette\Http\RequestFactory::fromGlobals');
        $response = $builder->addDefinition($this->prefix('response'))->setFactory(Nette\Http\Response::class);
        if ($config->cookiePath !== null) {
            $response->addSetup('$cookiePath', [$config->cookiePath]);
        }
        if ($config->cookieDomain !== null) {
            $value = ($config->cookieDomain === 'domain') ? $builder::literal('$this->getService(?)->getUrl()->getDomain(2)', [$request->getName()]) : $config->cookieDomain;
            $response->addSetup('$cookieDomain', [$value]);
        }
        if ($config->cookieSecure !== null) {
            $value = ($config->cookieSecure === 'auto') ? $builder::literal('$this->getService(?)->isSecured()', [$request->getName()]) : $config->cookieSecure;
    ...
    private function sendHeaders()
    {
        ...
        if (isset($config->frames) && $config->frames !== \true && !isset($headers['X-Frame-Options'])) {
        ...
        foreach (['csp', 'cspReportOnly'] as $key) {
            ...
            if (strpos($value, "'nonce'")) {
                ...
                $value = Nette\DI\ContainerBuilder::literal('str_replace(?, ? . $cspNonce, ?)', ["'nonce", "'nonce-", $value]);
            }
        ...
        if (!$config->disableNetteCookie) {
            $this->initialization->addBody('Nette\Http\Helpers::initCookie($this->getService(?), $response);', [$this->prefix('request')]);
        }
    }
    private static function buildPolicy(array $config): string
    {
        ...
        foreach ($config as $type => $policy) {
            ...
            $policy = ($policy === \true) ? [] : (array) $policy;
            $value .= $type;
