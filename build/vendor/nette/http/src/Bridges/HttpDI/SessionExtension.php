...
use FapiMember\Library\Nette\Http\IResponse;
...
class SessionExtension extends Nette\DI\CompilerExtension
{
    ...
    public function loadConfiguration()
    {
        ...
        $config = $this->config;
        $session = $builder->addDefinition($this->prefix('session'))->setFactory(Nette\Http\Session::class);
        ...
        if ($this->debugMode && $config->debugger) {
            $session->addSetup('@Tracy\Bar::addPanel', [new Nette\DI\Definitions\Statement(Nette\Bridges\HttpTracy\SessionPanel::class)]);
        }
