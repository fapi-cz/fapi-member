...
use FapiMember\Library\Doctrine\Common\Lexer\Token;
use FapiMember\Library\Egulias\EmailValidator\EmailLexer;
use FapiMember\Library\Egulias\EmailValidator\Warning\TLD;
...
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\DotAtEnd;
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\DotAtStart;
use FapiMember\Library\Egulias\EmailValidator\Warning\DeprecatedComment;
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\CRLFAtTheEnd;
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\LabelTooLong;
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\NoDomainPart;
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\ConsecutiveAt;
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\DomainTooLong;
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\CharNotAllowed;
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\DomainHyphened;
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\ExpectingATEXT;
use FapiMember\Library\Egulias\EmailValidator\Parser\CommentStrategy\DomainComment;
use FapiMember\Library\Egulias\EmailValidator\Result\Reason\ExpectingDomainLiteralClose;
use FapiMember\Library\Egulias\EmailValidator\Parser\DomainLiteral as DomainLiteralParser;
class DomainPart extends PartParser
{
    ...
    {
        $thereIsNoDomain = $this->lexer->current->isA(EmailLexer::S_EMPTY) || $this->lexer->current->isA(EmailLexer::S_SP) && !$this->lexer->isNextToken(EmailLexer::GENERIC);
        if ($thereIsNoDomain) {
    ...
    protected function doParseDomainPart(): Result
    {
        ...
        do {
            ...
            if ($this->lexer->current->isA(EmailLexer::S_OPENPARENTHESIS) || $this->lexer->current->isA(EmailLexer::S_CLOSEPARENTHESIS)) {
                $hasComments = \true;
    ...
    protected function checkDomainPartExceptions(Token $prev, bool $hasComments): Result
    {
        ...
        if ($this->lexer->current->isA(EmailLexer::S_BACKSLASH) && $this->lexer->isNextToken(EmailLexer::GENERIC)) {
