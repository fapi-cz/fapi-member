...
class Url implements \JsonSerializable
{
    ...
    /** @var array */
    public static $defaultPorts = ['http' => 80, 'https' => 443, 'ftp' => 21];
    /** @var string */
    private $scheme = '';
    /** @var string */
    private $user = '';
    ...
    {
        $this->query = is_array($query) ? $query + $this->query : self::parseQuery($this->getQuery() . '&' . $query);
        return $this;
    }
    ...
    {
        return ($this->host === '') ? '' : ((($this->user !== '') ? rawurlencode($this->user) . (($this->password === '') ? '' : (':' . rawurlencode($this->password))) . '@' : '') . $this->host . (($this->port && $this->port !== $this->getDefaultPort()) ? ':' . $this->port : ''));
    }
    /**
    ...
    public function isEqual($url): bool
    {
        ...
        return $url->scheme === $this->scheme && (!strcasecmp($host, $host2) || self::idnHostToUnicode($host) === self::idnHostToUnicode($host2)) && $url->getPort() === $this->getPort() && $url->user === $this->user && $url->password === $this->password && self::unescape($url->path, '%/') === self::unescape($this->path, '%/') && $query === $query2 && $url->fragment === $this->fragment;
    }
    /**
    ...
    {
        $this->path = preg_replace_callback('#[^!$&\'()*+,/:;=@%]+#', function (array $m): string {
    ...
    public static function unescape(string $s, string $reserved = '%;/?:@&=+$,'): string
    {
        ...
        if ($reserved !== '') {
            $s = preg_replace_callback('#%(' . substr(chunk_split(bin2hex($reserved), 2, '|'), 0, -1) . ')#i', function (array $m): string {
