...
class UrlImmutable implements \JsonSerializable
{
    ...
    protected function build(): void
    {
        ...
        $this->authority = ($this->host === '') ? '' : ((($this->user !== '') ? rawurlencode($this->user) . (($this->password === '') ? '' : (':' . rawurlencode($this->password))) . '@' : '') . $this->host . (($this->port && $this->port !== $this->getDefaultPort()) ? ':' . $this->port : ''));
    }
}
