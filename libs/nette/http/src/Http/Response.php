...
final class Response implements IResponse
{
    ...
    {
        self::checkHeaders();
        $options = ['expires' => $expire ? (int) DateTime::from($expire)->format('U') : 0, 'path' => $path ?? ($domain ? '/' : $this->cookiePath), 'domain' => $domain ?? ($path ? '' : $this->cookieDomain), 'secure' => $secure ?? $this->cookieSecure, 'httponly' => $httpOnly ?? \true, 'samesite' => $sameSite = $sameSite ?? self::SameSiteLax];
        if (\PHP_VERSION_ID >= 70300) {
            setcookie($name, $value, $options);
        } else {
            setcookie($name, $value, $options['expires'], $options['path'] . ($sameSite ? "; SameSite={$sameSite}" : ''), $options['domain'], $options['secure'], $options['httponly']);
        }
        return $this;
    }
    /**
    ...
    private function checkHeaders(): void
    {
        ...
        } elseif ($this->warnOnBuffer && ob_get_length() && !array_filter(ob_get_status(\true), function (array $i): bool {
            return !$i['chunk_size'];
        })) {
