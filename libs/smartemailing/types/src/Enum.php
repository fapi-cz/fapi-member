...
abstract class Enum
{
     ...
     * @return static
     */
    final public static function get(mixed $value): self
    {
     ...
     * @throws \Exception
     */
    public static function checkValue(mixed $value): void
    {
        ...
        if (!static::isValidValue($value)) {
            ...
            throw new InvalidTypeException(\sprintf('%s [%s] is not a valid value for %s, accepted values: %s', \is_object($value) ? \get_class($value) : Json::encode($value), \gettype($value), static::class, \implode(', ', $availableValues)));
        }
    }
    /**
     * @return array<mixed>
    ...
    {
        return \in_array($value, self::getAvailableValues(), \true);
    }
    ...
    private static function checkType(mixed $value): void
    {
     ...
     */
    private static function getDeclaredConstants(ReflectionClass $classReflection): array
    {
        $constants = $classReflection->getReflectionConstants();
        $className = $classReflection->getName();
        return \array_filter($constants, static fn(ReflectionClassConstant $constant): bool => $constant->getDeclaringClass()->getName() === $className);
    }
    private static function getValueIndex(mixed $value): string
    {
        $type = \gettype($value);
    ...
    private static function getEnumConstants(): array
    {
        ...
        $declaredPublicConstants = \array_filter($declaredConstants, static fn(ReflectionClassConstant $constant): bool => $constant->isPublic());
        $out = [];
