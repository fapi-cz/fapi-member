...
{
    final public static function from(mixed $value): bool
    {
        if (\is_bool($value)) {
            return $value;
        }
        ...
        if (\in_array($value, [\false, 0, '0', 'false'], \true)) {
            return \false;
        }
        if (\in_array($value, [\true, 1, '1', 'true'], \true)) {
            return \true;
        }
        throw InvalidTypeException::typeError('bool', $value);
    }
    final public static function fromOrNull(mixed $value, bool $nullIfInvalid = \false): ?bool
    {
        if ($value === null) {
            return null;
        }
     ...
     */
    final public static function extract($data, string $key): bool
    {
     ...
     * @throws \SmartEmailing\Types\InvalidTypeException
     */
    final public static function extractOrNull($data, string $key, bool $nullIfInvalid = \false): ?bool
    {
