...
use FapiMember\Library\Nette\Http\Url;
use FapiMember\Library\Nette\InvalidArgumentException;
...
final class UrlType implements ToStringInterface, ComparableInterface
{
    ...
    private function __construct(string $value)
    {
        ...
        $value = (string) \preg_replace_callback('/[^\x20-\x7f]/', static fn($match): string => \urlencode($match[0]), $value);
     ...
     */
    public function hasParameters(array $names): bool
    {
        $parameters = \array_keys($this->url->getQueryParameters());
        foreach ($names as $name) {
            if (!\in_array($name, $parameters, \true)) {
                return \false;
            }
        }
        return \true;
    ...
    {
        $dolly = clone $this;
        $dolly->url->setQueryParameter($name, $value);
        return $dolly;
    }
    /**
     * @deprecated use withHostName
     */
    public function withHost(Domain $host): self
    {
        $dolly = clone $this;
        $dolly->url->setHost($host->getValue());
        return $dolly;
    }
    public function withHostName(HostName $host): self
    {
        $dolly = clone $this;
        $dolly->url->setHost($host->getValue());
        return $dolly;
    ...
    {
        $dolly = clone $this;
        $dolly->url->setScheme($scheme);
        return $dolly;
    }
    public function withPath(string $path): self
    {
        $dolly = clone $this;
        $dolly->url->setPath($path);
        return $dolly;
    }
    ...
    private function addSlashToPathOrFail(string $value): ?string
    {
