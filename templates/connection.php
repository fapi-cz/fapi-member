...
<?php echo AlertProvider::showErrors(); ?>
    ...
    <?php echo FapiMemberTools::formStart('api_credentials_remove') ?>
        <table class="wp-list-table widefat fixed striped table-view-list">
            ...
            <?php
            $accounts = $apiService->getApiClients();
            if (empty($accounts) || $accounts[0]->getConnection()->getApiKey() === null){
                echo '<tr>
                        <td></td>
                ...
                foreach ($accounts as $account){
                    echo '<tr>
                            <td>'.$account->getConnection()->getApiUser().'</td>
                            <td>'.$account->getConnection()->getApiKey().'</td>
                            <td>'.
                                '<button class ="btn outline" name="fapiRemoveCredentials" type="submit" value='.$account->getConnection()->getApiKey().'>'.
    ...
    <p>
		...
		<?php echo ($apiService->areApiCredentialsSet()) ? '<span class="ok">' . __( 'propojeno', 'fapi') . '</span>' : '<span class="ng">' . __('nepropojeno', 'fapi-member' ) . '</span>' ?>
    </p>
    ...
    <p>
		<?php echo __( 'FAPI Member API Token', 'fapi-member' ); ?>: <span
                id="fapi-member-token"><?php echo get_option(OptionKey::TOKEN, '') ?></span>
